...
ESTADOS_VALIDOS = {"en_desarrollo", "buscando_financiamiento", "buscando_socio", "finalizado"}
...
async def list_proyectos(
    ...
    if actor_id is not None:
        ...
        conditions.append(f"EXISTS (SELECT 1 FROM proyecto_actor pa WHERE pa.proyecto_id = p.id AND pa.actor_id = ${len(args)})")
    if q:
        args.append(f"%{q}%")
    ...
    return [ProyectoList(**{**dict(r), "actor_ids": list(r["actor_ids"])}) for r in rows]
...
async def create_proyecto(
    ...
    # Si tiene TRL inicial, registrar en historial
...
async def get_proyecto(pid: int, db: asyncpg.Connection = Depends(get_db)):
    ...
    instrumentos = await db.fetch("""
        ...
        FROM proyecto_instrumento pi
        JOIN instrumento inst ON inst.id = pi.instrumento_id
...
async def add_actor(
    ...
    try:
        await db.execute("""
            INSERT INTO proyecto_actor (proyecto_id, actor_id, rol)
            VALUES ($1, $2, $3)
...
async def remove_actor(
    pid: int, actor_id: int,
    db: asyncpg.Connection = Depends(get_db),
):
    await _check_proyecto(pid, db)
    deleted = await db.fetchval("""
        DELETE FROM proyecto_actor
        WHERE proyecto_id = $1 AND actor_id = $2
        RETURNING 1
    """, pid, actor_id)
    if not deleted:
...
async def add_instrumento(
    pid: int, body: ProyectoInstrumentoAdd,
    db: asyncpg.Connection = Depends(get_db),
):
    await _check_proyecto(pid, db)
    inst = await db.fetchrow(
        "SELECT id, nombre, tipo FROM instrumento WHERE id = $1", body.instrumento_id
    )
    if not inst:
    ...
    try:
        await db.execute("""
            INSERT INTO proyecto_instrumento (proyecto_id, instrumento_id)
            VALUES ($1, $2)
...
async def remove_instrumento(
    pid: int, instrumento_id: int,
    db: asyncpg.Connection = Depends(get_db),
):
    await _check_proyecto(pid, db)
    deleted = await db.fetchval("""
        DELETE FROM proyecto_instrumento
        WHERE proyecto_id = $1 AND instrumento_id = $2
        RETURNING 1
    """, pid, instrumento_id)
    if not deleted:
        raise HTTPException(404, "Relación no encontrada")
...
async def _get_list_row(pid: int, db: asyncpg.Connection) -> ProyectoList:
    row = await db.fetchrow("""
        ...
        GROUP BY p.id, i.titulo
    """, pid)
    return ProyectoList(**{**dict(row), "actor_ids": list(row["actor_ids"])})
...
async def patch_zona(
    zona_id: int, body: ZonaPatch,
    db: asyncpg.Connection = Depends(get_db),
):
